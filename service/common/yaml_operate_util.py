import os
import yaml
from service.conf.setting import BASE_PATH, CASE_PATH, CONFIG_FILE, LOGIN_TOKEN_YAML_FILE, EXTACT_FILE
...
def read_testcase_yaml(yamlpath):
    with open(CASE_PATH + "/" + yamlpath, encoding="utf-8", mode="r") as f:
        value = yaml.load(f.read(), Loader=yaml.FullLoader)
        return value
